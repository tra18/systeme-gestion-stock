...
const StockPrediction = () => {
  ...
  const analyzeStockTrends = (stock, commandes) => {
    ...
    commandes.forEach(cmd => {
      if (cmd.createdAt?.toDate) {
        const month = cmd.createdAt.toDate().getMonth();
        if (!monthlyData[month]) monthlyData[month] = {};
        if (!monthlyData[month][cmd.article]) monthlyData[month][cmd.article] = 0;
        monthlyData[month][cmd.article] += cmd.quantite || 0;
      }
    });
  ...
  const generateAIRecommendations = (stock, commandes, analysis) => {
    ...
    stock.forEach(item => {
      ...
      const seasonalTrend = analyzeSeasonalTrend(item.nom, analysis.monthlyData);
      if (seasonalTrend) {
        recommendations.push({
          id: item.id,
          article: item.nom,
          type: 'seasonal',
          priority: 'medium',
          message: `Tendance saisonnière détectée`,
          recommendation: seasonalTrend.recommendation,
  ...
  const analyzeSeasonalTrend = (articleName, monthlyData) => {
    const articleData = {};
    Object.keys(monthlyData).forEach(month => {
      if (monthlyData[month][articleName]) {
        articleData[month] = monthlyData[month][articleName];
      }
    });

    if (Object.keys(articleData).length < 3) return null;
    ...
    if (coefficient > 0.3) {
      const peakMonth = Object.keys(articleData).reduce((a, b) => 
        articleData[a] > articleData[b] ? a : b
      );
      const monthNames = [
      ...
      return {
        recommendation: `Augmenter les stocks en ${monthNames[peakMonth]} (période de forte demande)`,
        confidence: 0.8
      };
    }
  ...
  const generateGlobalRecommendations = (analysis) => {
    ...
    // Recommandation sur l'optimisation des commandes
    const avgConsumption = Object.values(analysis.consumptionRates)
      .reduce((sum, rate) => sum + rate.dailyRate, 0) / Object.keys(analysis.consumptionRates).length;
    ...
    if (avgConsumption > 5) {
      recommendations.push({
        ...
        type: 'optimization',
        priority: 'medium',
        message: 'Consommation moyenne élevée détectée',
        recommendation: 'Considérer des commandes groupées pour réduire les coûts',
        confidence: 0.8,
        impact: 'medium'
      });
    }
